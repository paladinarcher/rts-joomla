...
class RtsusersApiResourceGetuser extends ApiResource {
    ...
    public function createUser($server, $fromRemote) {
        ...
        if (!$instance->save()) { 
            ...
            throw new Exception("No email address specified ({$fromRemote->user->email}), can't register without one!");
        }
    ...
    protected function linkMe($user_id, $fromRemote, $server) {
        ...
        return $this->link($user->id, $fromRemote->user->id, $server)->loginUser($user, $server, $fromRemote->user->sessionkey);
    }
